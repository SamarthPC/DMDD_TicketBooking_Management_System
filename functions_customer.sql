...
RETURN NUMBER
IS
    l_addon_array NUMBER_ARRAY;
    l_quantity_array NUMBER_ARRAY;
    l_total_price NUMBER := 0;
BEGIN
    l_addon_array := STRING_TO_TABLE(p_addon_list, ',');
    l_quantity_array := STRING_TO_TABLE(p_quantity_list, ',');
    FOR i IN 1..l_addon_array.COUNT LOOP
        l_total_price := l_total_price + CalculateAddOnPrice(l_addon_array(i), l_quantity_array(i));
    END LOOP;
...
RETURN NUMBER
IS
    l_addon_array NUMBER_ARRAY := STRING_TO_TABLE(p_addon_list, ',');
    l_quantity_array NUMBER_ARRAY := STRING_TO_TABLE(p_quantity_list, ',');
    l_rows_inserted NUMBER := 0;
BEGIN
    FOR i IN 1..l_addon_array.COUNT LOOP
        INSERT INTO customer_addon (addon_id, addon_quantity, ticket_id)
        VALUES (l_addon_array(i), l_quantity_array(i), p_ticket_id);
...
AS
  v_payment_id NUMBER;
BEGIN
  INSERT INTO payment(name_on_card, address, payment_amount,payment_status)
  VALUES (p_name_on_card,p_address,p_payment_amount,'Successful')
