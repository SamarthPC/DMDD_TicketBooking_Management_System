...
CREATE OR REPLACE FUNCTION checkavailability(screen_id IN NUMBER, v_seat_list IN VARCHAR2) RETURN NUMBER
IS
BEGIN

    DECLARE
        v_seat_numbers_arr VARCHAR2_ARRAY := STRING_TO_TABLE(v_seat_list, ',');
    ...
    BEGIN
        ...
        FOR i IN 1..v_seat_numbers_arr.COUNT LOOP
            -- Get the seat status based on the seat number and screen ID
            SELECT seat_status INTO v_seat_status 
            FROM seat 
            WHERE seat_number = v_seat_numbers_arr(i) AND screen_id = screen_id;
            
            -- If no seat is found, raise an error
            IF v_seat_status IS NULL THEN
                RAISE_APPLICATION_ERROR(-20002, 'Seat ' || v_seat_numbers_arr(i) || ' not found');
            END IF;
            
            IF v_seat_status = 'N' THEN
                RAISE_APPLICATION_ERROR(-20003, 'Seat ' || v_seat_numbers_arr(i) || ' is occupied. Select another seat');
            END IF;
        END LOOP;
